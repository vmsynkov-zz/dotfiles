...
mkdir -p $SYSTEMD
for entry in $(ls --almost-all config/systemd/user)
do 
if [ ! -a $SYSTEMD/$entry ]; then
install -m 0644 -p -t $SYSTEMD config/systemd/user/$entry
fi
done
...
for entry in $(ls --almost-all config --ignore oh-my-zsh --ignore wallpapers --ignore nvim --ignore systemd); do 
